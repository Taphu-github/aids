...
type MQTTCredit = typeof initialMQTTCredits[0]

export default function MQTTCreditTable() {
  const [mqttCredits, setMQTTCredits] = useState(initialMQTTCredits)
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [currentMQTTCredit, setCurrentMQTTCredit] = useState<MQTTCredit | null>(null)

  const handleOpenDialog = (mqttCredit: MQTTCredit | null = null) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const mqttCreditData = Object.fromEntries(formData.entries()) as unknown as MQTTCredit
